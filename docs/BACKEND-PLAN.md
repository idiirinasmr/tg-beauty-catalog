...
| **Админ (ты)** | Все мастера, все платежи, статистика | Ручная активация подписок, управление платформой, уведомления и отчёты через бот в личку |
...
|   |   |   |-- admin-bot.js   # Бот владельца платформы (уведомления + команды)
...
20. Бот владельца платформы: admin-bot.js — подключение бота, команды /stats, /masters, /today
21. Уведомление владельцу при регистрации нового мастера

### Этап 4: Mini App -> API
22. Переделать tg-app/app.js: вместо хардкода services/master -> fetch GET /api/catalog/:botUsername
23. Определение бота: Mini App берёт bot_username из URL-параметра ?bot=
24. POST /api/booking: сохранение в БД + уведомление мастеру через бота
25. Валидация initData на сервере (middleware validate-init-data.js)
26. Skeleton-экран при загрузке данных с API (пока данные не пришли)

### Этап 5: Бот-консультант + очередь сообщений
27. Установить Redis на VPS: `apt install redis-server`
28. Настроить BullMQ: server/src/utils/queue.js (3 очереди: broadcast, reminder, notification)
29. FAQ-движок: faq-engine.js — поиск по ключевым словам в таблице faq
30. Переадресация на мастера: если FAQ не нашёл ответ -> пересылает сообщение мастеру
31. Напоминания: cron каждые 5 мин проверяет bookings с reminder_sent=false, ставит задачу в очередь
32. Worker рассылок: берёт задачи из очереди, отправляет с лимитом 20 msg/sec на бота
33. Обработка 429 (Too Many Requests): автоматический retry с exponential backoff
34. Сбор отзывов: при статусе booking=completed -> бот спрашивает оценку через 2 часа

### Этап 6: Подписка и платежи
35. Интеграция ЮKassa: POST /api/payment/create -> redirect на страницу оплаты
36. Вебхук ЮKassa: POST /api/payment/webhook -> проверка подписи -> активация подписки
37. Интеграция Stripe (аналогично ЮKassa — для зарубежных мастеров)
38. Ручная активация: POST /api/payment/manual -> админ подтверждает -> обновление plan='pro'
39. Middleware plan-check.js: проверка plan + plan_expires_at на каждом запросе к pro-функциям
40. Cron: ежедневная проверка просроченных подписок -> downgrade до free
41. Уведомления владельцу об оплатах и просроченных подписках (admin-bot.js)
42. Ежедневный отчёт владельцу в 21:00: мастера, клиенты, выручка, истекающие подписки

### Этап 7: Админ-панель мастера
43. Веб-страница admin/index.html: авторизация через Telegram Login Widget
44. Разделы: профиль, услуги (CRUD), портфолио (загрузка фото), заявки, клиентская база (pro)
45. Настройки (pro): загрузка логотипа, смена названия в шапке, выбор цветовой темы
46. Рассылки (pro): форма текст + фото -> POST /api/master/broadcast -> очередь BullMQ
47. Статистика: записей за месяц, новых клиентов, конверсия (открыл каталог -> записался)

### Этап 8: Админ-панель платформы (для тебя)
48. Отдельная веб-страница (защищена по ADMIN_TELEGRAM_IDS)
49. Список всех мастеров: имя, тариф, количество клиентов, дата регистрации
50. Ручная активация/деактивация подписки
51. Общая статистика: мастеров, клиентов, записей, доход
52. Список платежей: кто, когда, сколько, статус
53. Мониторинг: потребление RAM/CPU, количество активных ботов, размер очереди BullMQ
54. Мониторинг бэкапов: уведомление в бот владельца если бэкап не прошёл
...
ADMIN_BOT_TOKEN=xxx              # Токен бота-уведомителя для владельца платформы
```
...
## 15. Бот владельца платформы (уведомления в личку)

### Зачем

Чтобы не заходить каждый раз на разные платформы — вся информация о платформе приходит тебе прямо в Telegram. Отдельный бот, который работает только для тебя (по `ADMIN_TELEGRAM_IDS`).

### Мгновенные уведомления

| Событие | Сообщение |
|---------|-----------|
| Новый мастер | «Новый мастер: Анна (@anna_beauty). Всего мастеров: 12/50» |
| Оплата Pro | «Оплата 500 руб от Анна (ЮKassa). Выручка за месяц: 3500 руб» |
| Подписка истекает через 3 дня | «Через 3 дня истекает Pro у Анна. Клиентов: 47» |
| Подписка просрочена | «Pro просрочен у Анна. Переведена на Free» |
| Мастер отключил бота | «Мастер Анна удалил бота. Токен недействителен» |
| Бэкап не прошёл | «Ошибка бэкапа: файл не создан. Проверь VPS» |

### Команды бота

| Команда | Ответ |
|---------|-------|
| `/stats` | Мастеров: 12 (Free: 7, Pro: 5). Клиентов: 340. Выручка за месяц: 2500 руб |
| `/today` | Сегодня: +2 мастера, +15 клиентов, 1 оплата (500 руб) |
| `/masters` | Список всех мастеров: имя, тариф, кол-во клиентов |
| `/master Анна` | Карточка мастера: услуги, клиенты, записи, тариф, дата регистрации |
| `/revenue` | Выручка: сегодня / неделя / месяц / всего |
| `/expiring` | Мастера, у кого подписка истекает в ближайшие 7 дней |
| `/activate Анна` | Вручную активировать Pro для мастера |
| `/backup` | Статус последнего бэкапа: дата, размер, ОК/ошибка |

### Ежедневный отчёт (автоматически в 21:00)

```
📊 Отчёт за 14.03.2026:

Мастеров: 12 (Free: 7, Pro: 5)
Новых сегодня: +2
Клиентов всего: 340 (+15 сегодня)
Записей сегодня: 28
Выручка сегодня: 500 руб
Выручка за месяц: 3500 руб
Подписки истекают скоро: 1 (Мария, через 3 дня)
```

### Реализация

- **Бот:** отдельный Telegram-бот (создать через BotFather), токен в `ADMIN_BOT_TOKEN`
- **Файл:** `server/src/services/admin-bot.js`
- **Доступ:** только `ADMIN_TELEGRAM_IDS` (все остальные получают «Бот не предназначен для использования»)
- **Ежедневный отчёт:** cron-задача в 21:00 (node-cron)
- **Уведомления:** вызываются из соответствующих обработчиков (register, payment webhook, cron подписок)

### Когда внедрять

- Базовые команды (`/stats`, `/masters`, `/today`) — **Этап 3** (вместе с подключением ботов)
- Уведомления об оплатах — **Этап 6** (вместе с платежами)
- Ежедневный отчёт — **Этап 6** (когда есть данные о платежах)
- Мониторинг бэкапов — **Этап 8** (финальная настройка)

---

## 16. Главное правило архитектуры
